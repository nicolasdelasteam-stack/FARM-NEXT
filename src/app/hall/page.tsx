  ...
  return (
    <div className="max-w-3xl">
      <div className="flex justify-between items-center mb-1">
        ...
        <button onClick={() => setOpen(!open)} className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-sm font-semibold">{open ? 'Fechar' : '+ Troféu'}</button>
      </div>
      ...
      {open && (
        <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2 mb-5">
          <div className="flex gap-1.5 flex-wrap">
            {EMO.map((e) => <button key={e} onClick={() => setIcon(e)} className={`w-9 h-9 rounded-lg text-lg ${icon === e ? 'bg-indigo-600' : 'bg-zinc-800'}`}>{e}</button>)}
          </div>
          ...
          <button onClick={addTrofeu} className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-sm font-semibold">Adicionar ao Hall</button>
        </div>
      )}
            ...
            <div key={h.id} className={`p-4 rounded-xl border ${done ? 'bg-gradient-to-br from-yellow-950/40 to-zinc-900 border-yellow-800/40' : 'bg-zinc-900 border-zinc-800'}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] uppercase tracking-wide text-indigo-400">{TIPO_LABEL[h.tipo]}</div>
