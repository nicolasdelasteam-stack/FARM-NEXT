    ...
    <div className="max-w-3xl">
      ...
      <div className="flex gap-2 mb-5">
        {(['materias', 'biblioteca'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${tab === t ? 'bg-violet-600 text-white' : 'bg-zinc-900 text-zinc-400'}`}>{t === 'materias' ? 'Matérias' : 'Bibliotheca'}</button>
        ))}
      </div>

      {tab === 'materias' && (
        <div>
          <div className="flex justify-end mb-3"><button onClick={() => setOpen(!open)} className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm font-semibold">{open ? 'Fechar' : '+ Matéria'}</button></div>
          {open && (
            <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2 mb-4">
              <div className="flex gap-1.5 flex-wrap">{EMO.map((e) => <button key={e} onClick={() => setEmoji(e)} className={`w-9 h-9 rounded-lg text-lg ${emoji === e ? 'bg-violet-600' : 'bg-zinc-800'}`}>{e}</button>)}</div>
              ...
              <button onClick={addMateria} className="w-full py-2 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm font-semibold">Adicionar matéria</button>
            </div>
          )}
              ...
              <div className="space-y-2">
                {estudos.materias.filter((m) => m.dia === d).sort((a, b) => a.horario.localeCompare(b.horario)).map((m) => (
                  <div key={m.id} className="p-3 rounded-xl bg-zinc-900 border border-zinc-800">
                    ...
                    <textarea className="w-full mt-2 bg-zinc-800/60 border border-zinc-700/50 rounded-lg px-2 py-1 text-xs outline-none focus:border-violet-500" rows={2} placeholder="Resumo / anotações..." value={m.resumo} onChange={(e) => setResumo(m.id, e.target.value)} />
                  </div>
                ))}
              </div>
            </div>
      ...
      {tab === 'biblioteca' && (
        <div>
          <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 mb-4">
            ...
            <div className="flex gap-2"><input className={inp} placeholder="Link do PDF (opcional)" value={lLink} onChange={(e) => setLLink(e.target.value)} /><button onClick={addLivro} className="px-4 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm font-semibold">+</button></div>
          </div>
          {tipos.map((tp) => (
                ...
                {estudos.biblioteca.filter((l) => l.tipo === tp).map((l) => (
                  <div key={l.id} className="p-3 rounded-xl bg-zinc-900 border border-zinc-800">
                    <div className="flex justify-between items-start">
                      <div className="font-semibold text-sm">{l.link ? <a href={l.link} target="_blank" rel="noopener" className="text-violet-300 hover:underline">{l.titulo}</a> : l.titulo}</div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <input type="range" min={0} max={100} value={l.progresso} onChange={(e) => updLivro(l.id, { progresso: parseInt(e.target.value) })} className="flex-1 accent-violet-500" />
