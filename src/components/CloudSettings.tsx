  ...
  if (!cloudConfigured()) {
    return (
      <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800">
        ...
        <p className="text-xs text-zinc-500">Defina <code className="text-violet-300">NEXT_PUBLIC_SUPABASE_URL</code> e <code className="text-violet-300">NEXT_PUBLIC_SUPABASE_ANON_KEY</code> no <code>.env.local</code> para sincronizar entre dispositivos. O SQL da tabela está no README.</p>
      </div>
    );
  }
  return (
    <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800">
      ...
      {msg && <p className="text-xs text-violet-300 mb-2">{msg}</p>}
      {user ? (
        <div className="space-y-2">
          <p className="text-xs text-zinc-400">Logado como <b>{user}</b></p>
          <div className="flex gap-2 flex-wrap">
            <button onClick={() => saveToCloud().then(() => flash('Salvo na nuvem ✅')).catch((e: Error) => flash(e.message))} className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm font-semibold">Salvar na nuvem</button>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => signInEmail(email).then(() => flash('Link mágico enviado ao seu email')).catch((e: Error) => flash(e.message))} className="px-3 py-1.5 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm font-semibold">Entrar</button>
        </div>
      )}
    </div>
  );
